...
xhook.before(function (request) {
    /*const CONVERSATION_URL = "https://beta.character.ai/chat/streaming/";
    if (request.url === CONVERSATION_URL) {
...
xhook.after(function (request, response) {
    const HISTORIES_URL = "https://beta.character.ai/chat/character/histories/";
    const HISTORIES_INFO = "https://beta.character.ai/chat/character/info/";
    if (request.url !== HISTORIES_URL &&
        request.url !== HISTORIES_INFO) {
        return;
    }
    ...
    if (request.url === HISTORIES_URL) {
        ...
        const fetchMessages = async (nextPage, chatExternalId, chat) => {
            ...
            await fetch(url, {
                ...
                .then(async (data) => {
                    ...
                    if (data.has_more) {
                        await fetchMessages(data.next_page, chatExternalId, chat);
                    }
                });
        };
        const fetchAllMessages = async () => {
            for (const chat of jsonData.histories) {
                ...
                await fetchMessages(0, chatExternalId, chat);
            }
    ...
    else if (request.url === HISTORIES_INFO) {
        const jsonData = JSON.parse(response.text);
    ...
    if (request.url.includes(CONVERSATION_URL) && response.status === 200) {
        ...
        const jsonData = JSON.parse(response.text);
        if (jsonData != null) {
            if (window.localStorage.getItem('cai_conversation_' + historyExtId)) {
                window.localStorage.removeItem('cai_conversation_' + historyExtId)
            }
            window.localStorage.setItem('cai_conversation_' + historyExtId, JSON.stringify(jsonData));
        } else {
            alert("Failed to intercept CAI. Try reloading.");
        }
    }*/
...
/*
            ...
            if (!document.querySelector('meta[cai_token]')){
                createMeta_token(AccessToken);
            }
function createMeta_token(AccessToken){
    const meta = document.createElement('meta');
...
fetch("https://beta.character.ai/chat/history/msgs/user/?history_external_id=nM5-eWVxQzAnUrEUOTqorOa8YFuEszy-ZinJn8Ilhj8", {
    method: "POST",
    body: `
        'history_external_id': 'blabla',
