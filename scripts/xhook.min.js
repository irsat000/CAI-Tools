...
xhook.before(function (request) {
    ...
    if (request.url.includes(HISTORIES_URL)) {
        ...
        if (!charId || !checkMemoryManager(charId)) return;
        // Get body
        let payload = JSON.parse(request.body);
        // Alter payload's text property (user's message)
        const message = payload.text;
        payload.text = addMemoryToMessage(message, charId);
        // Save body
        request.body = JSON.stringify(payload);
        return;
    }
...
function checkMemoryManager(charId, type) {
    try {
        // No memory settings
        ...
        if (type === "getList") {
            ...
            // Checks if memory manager is active
...
function addMemoryToMessage(original, charId) {
    try {
        ...
        let memoryPart = "";
        const charMemories = checkMemoryManager(charId, "getList");
        if (!charMemories || !Array.isArray(charMemories))
            throw "Character's memories couldn't be found. " + charMemories;
        // Push the memories
...
xhook.after(function (request, response) {
    const HISTORIES_URL = "character.ai/chat/character/histories_v2/";
    const HISTORIES2_URL = "neo.character.ai/chats/?character_ids=";

    const CHAT_URL = "character.ai/chat/history/msgs/user/?history_external_id=";
    const CHAT2_URL = "neo.character.ai/turns/";

    const USER_URL = "character.ai/chat/user/";
    if (!request.url.includes(HISTORIES_URL) &&
        !request.url.includes(HISTORIES2_URL) &&
        !request.url.includes(CHAT_URL) &&
        !request.url.includes(CHAT2_URL) &&
        !request.url.includes(USER_URL)) {
        return;
    }
    ...
    if (request.url.includes(HISTORIES_URL)) {
        try {
            ...
            if (jsonData != null && Object.keys(jsonData).length !== 0 && charId != null) {
                const chatIdList = JSON.stringify(jsonData.histories.map(chat => chat.external_id));

                if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                    document.querySelector('meta[cai_charid="' + charId + '"]')
                        .setAttribute('cai_history1_chatlist', chatIdList);
                }
                else {
                    const meta = document.createElement('meta');
                    meta.setAttribute('cai_charid', charId);
                    meta.setAttribute('cai_history1_chatlist', chatIdList);
                    document.head.appendChild(meta);
                }
            }
        } catch (error) {
            console.log("Error at CAI Tools", error);
        }
    }
    else if (request.url.includes(HISTORIES2_URL) && request.method === "GET") {
        try {
            const AccessToken = request.headers.Authorization;
            createAccessTokenMeta(AccessToken);
            const url = new URL(request.url);
            const searchParams = new URLSearchParams(url.search);
            const charId = searchParams.get('character_ids');
            const jsonData = JSON.parse(response.text);

            if (jsonData != null && Object.keys(jsonData).length !== 0 && charId != null) {
                const chatIdList = JSON.stringify(jsonData.chats.map(chat => chat.chat_id));

                if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                    document.querySelector('meta[cai_charid="' + charId + '"]')
                        .setAttribute('cai_history2_chatlist', chatIdList);
                }
                else {
                    const meta = document.createElement('meta');
                    meta.setAttribute('cai_charid', charId);
                    meta.setAttribute('cai_history2_chatlist', chatIdList);
                    document.head.appendChild(meta);
                }
            }
        } catch (error) {
    ...
    else if (request.url.includes(CHAT_URL)) {
        try {
            const AccessToken = request.headers.Authorization;
            createAccessTokenMeta(AccessToken);
            const url = new URL(request.url);
            ...
            if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                document.querySelector('meta[cai_charid="' + charId + '"]')
                    .setAttribute('cai_currentConverExtId', historyExtId);
            }
            else {
                const meta = document.createElement('meta');
                meta.setAttribute('cai_charid', charId);
                meta.setAttribute('cai_currentConverExtId', historyExtId);
                document.head.appendChild(meta);
            }
        } catch (error) {
            console.log("Error at CAI Tools", error);
    ...
    else if (request.url.includes(CHAT2_URL) && request.method === "GET") {
        try {
            const AccessToken = request.headers.Authorization;
            createAccessTokenMeta(AccessToken);

            const url = new URL(request.url);
            const historyExtId = getPathPart(2, url.pathname);
            const charId = getCharId();
    ...
    else if (request.url.includes(USER_URL) && request.method === "GET") {
        try {
            const AccessToken = request.headers.Authorization;
            createAccessTokenMeta(AccessToken);
            ...
            if (jsonData != null && Object.keys(jsonData).length !== 0) {
                ...
                } else if (jsonData.user && jsonData.user.username) {
                    userName = jsonData.user.username;
                } else if (jsonData.user && jsonData.user.account && jsonData.user.account.name) {
                    userName = jsonData.user.account.name;
                }

                if (document.querySelector('meta[cait_user]')) {
                    document.querySelector('meta[cait_user]')
                        .setAttribute('cait_user', userName);
                }
                else {
                    const meta = document.createElement('meta');
                    meta.setAttribute('cait_user', userName);
                    document.head.appendChild(meta);
                }
            }
        } catch (error) {
...
function getPathPart(index, pathname) {
