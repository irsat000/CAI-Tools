...
xhook.after(function (request, response) {
    ...
    if (request.url === HISTORIES_URL && response.status === 200) {
        ...
        if (jsonData != null && jsonData.length > 0) {
            localStorage.removeItem('cai_history')
            localStorage.setItem('cai_history', JSON.stringify(jsonData));
        }
    }
    ...
    if (request.url === INFO_URL && response.status === 200) {
        ...
        if (jsonData != null) {
            localStorage.removeItem('cai_info')
            localStorage.setItem('cai_info', JSON.stringify(jsonData));
        }
    }
});