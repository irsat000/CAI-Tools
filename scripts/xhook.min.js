...
xhook.after(function (request, response) {
    ...
    const HISTORIES_INFO_CACHED = "https://beta.character.ai/chat/character/info-cached/";
    ...
    else if(request.url === HISTORIES_INFO || request.url.includes(HISTORIES_INFO_CACHED)){
        const jsonData = JSON.parse(response.text);
        const charId = jsonData.character.external_id;
        if (jsonData != null) {
            if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                document.querySelector('meta[cai_charid="' + charId + '"]')
                    .setAttribute('cai_settings', JSON.stringify(jsonData));
            }
            else {
                const meta = document.createElement('meta');
                meta.setAttribute('cai_charid', charId);
                meta.setAttribute('cai_settings', JSON.stringify(jsonData));
                document.head.appendChild(meta);
            }
        } else {
            alert("Failed to intercept CAI. Try reloading.");
