...
xhook.before(function (request) {
    /*const CHAT_URL = "https://beta.character.ai/chat/streaming/";
    if (request.url === CHAT_URL) {
...
xhook.after(function (request, response) {
    ...
    const HISTORIES_INFO = "character.ai/chat/character/info/";
    ...
    if (!request.url.includes(HISTORIES_URL) &&
        ...
        !request.url.includes(HISTORIES_INFO) &&
    ...
    if (request.url.includes(HISTORIES_URL)) {
        ...
        if (jsonData != null && charId != null) {
            if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                document.querySelector('meta[cai_charid="' + charId + '"]')
                    .setAttribute('cai_temphistory', JSON.stringify(jsonData));
            }
            else {
                const meta = document.createElement('meta');
                meta.setAttribute('cai_charid', charId);
                meta.setAttribute('cai_temphistory', JSON.stringify(jsonData));
        ...
        if (jsonData != null && charId != null) {
            if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                document.querySelector('meta[cai_charid="' + charId + '"]')
                    .setAttribute('cai_temphistory2', JSON.stringify(jsonData));
            }
            else {
                const meta = document.createElement('meta');
                meta.setAttribute('cai_charid', charId);
                meta.setAttribute('cai_temphistory2', JSON.stringify(jsonData));
                document.head.appendChild(meta);
            }
        }
    }
    else if (request.url.includes(HISTORIES_INFO)) {
        const AccessToken = request.headers.Authorization;
        createAccessTokenMeta(AccessToken);
        const jsonData = JSON.parse(response.text);
        const charId = jsonData.character.external_id;
        if (jsonData != null) {
            if (document.querySelector('meta[cai_charid="' + charId + '"]')) {
                document.querySelector('meta[cai_charid="' + charId + '"]')
                    .setAttribute('cai_info', JSON.stringify(jsonData));
            }
            else {
                const meta = document.createElement('meta');
                meta.setAttribute('cai_charid', charId);
                meta.setAttribute('cai_info', JSON.stringify(jsonData));
                document.head.appendChild(meta);
            }
        } else {
        ...
        if (document.querySelector('meta[cai_currentConverExtId]')) {
            document.querySelector('meta[cai_currentConverExtId]')
        ...
        if (historyExtId != null && charId != null) {
            if (document.querySelector('meta[cai_currentConverExtId="' + historyExtId + '"]')) {
                document.querySelector('meta[cai_currentConverExtId="' + historyExtId + '"]')
                    .setAttribute('cai_currentConverExtId', historyExtId);
            }
            else {
                const meta = document.createElement('meta');
