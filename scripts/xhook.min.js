...
xhook.after(function (request, response) {
    ...
    if (request.url === HISTORIES_URL && response.status === 200) {
        ...
        if (jsonData != null) {
            if (window.localStorage.getItem('cai_history_' + charId)) {
                window.localStorage.removeItem('cai_history_' + charId)
            }
            try {
                window.localStorage.setItem('cai_history_' + charId, JSON.stringify(jsonData));
            } catch (error) {
                clearStorage_CaiTools();
                try {
                    window.localStorage.setItem('cai_history_' + charId, JSON.stringify(jsonData));
                } catch (error) {
                    localStorage.clear();
    ...
    if (request.url === INFO_URL && response.status === 200) {
        ...
        if (jsonData != null) {
            if (window.localStorage.getItem('cai_info_' + charId)) {
                window.localStorage.removeItem('cai_info_' + charId)
            }
            try {
                window.localStorage.setItem('cai_info_' + charId, JSON.stringify(jsonData));
            } catch (error) {
                clearStorage_CaiTools();
                try {
                    window.localStorage.setItem('cai_info_' + charId, JSON.stringify(jsonData));
...
function clearStorage_CaiTools() {
    console.log("Cleaning the data stored by CAI Tools")
    for (let i = 0; i < window.localStorage.length; i++) {
        const key = window.localStorage.key(i);
        if (key.includes("cai_conversation_") || key.includes("cai_info_") || key.includes("cai_history_")) {
